...
RUN npm install
WORKDIR /app/shared-modules/metering-helper
RUN cd /app/shared-modules/metering-helper
RUN npm install
#-------------------
#Setup Working Directory
WORKDIR /app/product-manager/
#Set Sevice Config Directory
ENV NODE_CONFIG_DIR=../shared-modules/config-helper/config/
#Install App
RUN cd /app/product-manager/
RUN npm install
#Start App
CMD ["node", "/app/product-manager/server.js"]
#expose a port
EXPOSE 3006